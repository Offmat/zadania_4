...
SELECT name, price*100 AS "price (gr)" FROM products;
...
-- Napisz mi proszę czy dobrze rozkminiłem wcięcia, bo robiłem na czuja z przykłądów z zajęć
SELECT *
FROM products
...
SELECT manufacturer_id, AVG(price) AS "średnia cena"
FROM products
  INNER JOIN manufacturers
    ON products.manufacturer_id = manufacturers.id
...
SELECT manufacturers.name, AVG(price) AS "średnia cena"
FROM products
  INNER JOIN manufacturers
    ON products.manufacturer_id = manufacturers.id
